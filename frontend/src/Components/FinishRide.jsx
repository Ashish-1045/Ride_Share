...
const FinishRide = (props) => {
  ...
  async function endRide() {
    try {
      ...
      if (response.status === 200) {
        props.setFinishRidePanal(false);
  ...
  return (
    <div className="bg-gray-100 w-full h-screen ">
      ...
      >
        <i className="text-gray-300 mt-4 ri-arrow-down-wide-fill"></i>
      </h3>

      <div className="flex items-center justify-start ml-2 w-full ">
        <h3 className="text-2xl font-bold text-center ml-2">Ride Finish</h3>
      </div>
        ...
        <div className="border-b-2 pb-4 space-y-9 w-full border-gray-500 mt-5">
          <div className="flex items-center gap-6 ml-4 h-auto">
            ...
            <div className="flex flex-col ">
              ...
              <h4 className="font-semibold"> {props.ride?.pickup}</h4>
            </div>
          </div>
        </div>
      ...
      <button
        className="flex justify-center bg-orange-400 text-black py-3 px-26 rounded-lg text-xl font-semibold hover:bg-orange-600 transition duration-300 w-full mt-4"
        onClick={async () => {
          await endRide();
          props.onRideCompleted();
        }}
      >
        Completed
      </button>
